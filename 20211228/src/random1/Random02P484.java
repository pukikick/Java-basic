...
public class Random02P484 {
	...
	public static void main(String[] args) {
		...
		// 동전던지기 카운팅(count) 프로그램을 만들어주세요.
		// n회 던졌을때 true가 나오면 앞면, false가 나오면 뒷면으로 간주해서
		// 프러그램이 다 끝낫을떼 앞면 몇 회, 뒷면 몇 회 인지
		// 콘솔에 출력해주세요.
        ...
        for(int i=0;i<count;i++) {// 0 부터 count범위 만큼
        	// random.nextBoolean( )이 ture면 앞면 1 증가
    		// false면 뒷면 1 증가하도록 조건식 작성
    		boolean result = random.nextBoolean();
    		if(result) { // 참이니 true이고
    			front++;
    		} else { // 거짓이니 false이다
    			back++;
    		}
		...
		// 다 돌고나서 돌린 결과값 콘솔찍기
        System.out.println(count+"바튀 돌린 결과값");
