-- ict_practice를 쓰도록 해 주세오.
use ict_practice;

-- 앞에 스키마명을 추가로 입력해 스키마명.테이블명으로 조회도 가능합니다.
SELECT*FROM usertbl;

-- 유저를 두 명만 더 추가해주세요.
-- 정보는 전부 여러분들이 알아서 집어넣어주시되, 키는 한 명은 160대로 해주세요.
INSERT INTO userTbl VALUES ('KDH', '씨맥', 1990, '서울', '01010101010', 165, '2021-05-05');
INSERT INTO userTbl VALUES ('LSH', '페이커', 1997, '서울', '01012345678', 179, '2021-05-04');
...
-- hint : 91보다는 크거나 같고, '그리고', 99보다는 작거나 같다.
 ...
 SELECT * FROM userTbl WHERE addr = '서울' or '영국';
 -- 또는 SELECT * FROM userTbl WHERE (addr = '서울') or (addr='영국'); 가능
 
 -- int키워드를 사용하면 컬럼명 in (데이터1, 2, 3, 4...);
...
-- 서브쿼리를 활용해 '페이커' 보단 먼저 태어난 사람들만 골라내보세요.
-- 1. 페이커의 생년을 얻는 쿼리문부터 작성합니다.
SELECT bitrh_year FROM usertbl WHERE user_name='페이커';
...
SELECT user_name, birth_year FROM usertbl WHERE birth_year <
    (SELECT birth_year FROM usertbl WHERE user_name='페아커');
...
-- 2021년 가입자중 생년이 제일 빠른 사람보다 키가 큰 사람을 조회해주세요.
-- 날짜도 부등호로 조회 가능합니다.(작다 : 이전 날짜, 크다 : 이후날짜)
...
-- 누구인지는 모르지만 제일 작은 키를 구해준다.
SELECT min(height) FROM usertbl WHERE user_id like '%M%';    

-- 2021년 가입자중 가입일이 제일 느린 사람보다 키가 큰 사람을 조회해주세여.
-- 날짜도 부등호로 조회 가능합니다. (작다 : 이전 날짜, 크다 : 이후날짜)

-- 1. 2021년 가입자중 가입일이 제일 빠른 사람의 가입일자 구하기
SELECT min(reg_date) FROM usertbl WHERE reg_date >= '2021-01-01';
-- 2. 1의 쿼리문을 조건으로 가입일이 제일 빠른 사람의 키 구하기.
...
-- 유저를 다섯 명 더 추가하겠습니다.
INSERT INTO userTbl VALUES ('KJV', '김자바', 1983, '서울', '011111111', 171, '2020-08-15');
INSERT INTO userTbl VALUES ('ADR', '압둘라', 1995, '경기', '022222222', 183, '2021-04-01');
INSERT INTO userTbl VALUES ('YSO', '야스오', 2001, '부산', '033333333', 165, '2021-10-08');
INSERT INTO userTbl VALUES ('ZYA', '장위안', 1985, '부산', '044444444', 164, '2020-02-28');
INSERT INTO userTbl VALUES ('SPR', '스프링', 1987, '강원', '055555555', 184, '2021-12-31');
INSERT INTO userTbl VALUES ('JSP', '자스파', 1989, '전라', '066666666', 177, '2022-01-01');
...
-- height을 조건에 잡아줄지 모호하기 떄문에 에러가 납니다.
SELECT * FROM usertbl WHERE height <
   (SELECT min(height)FROM usertbl WHERE addr='서울');

-- ANY 구문을 사용ㅎ면 169, 171, 186 모든 데이터에 대해 OR로 처리가 됩니다.
-- 개별값 모구에 대해 OR처리가 붙고 그래서 아래와 같이 
-- (height < 169) OR (height < 171) OR (HEIGHT < 186) 
-- 3개 조건이 OR로 연결됩니다.
-- ANY는 OR로 연결된다는 특성상 범위가 가장 넓은 조건 하나로 동일됩니다.
